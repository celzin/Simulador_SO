...
PCB::PCB(int id, const std::vector<Instruction>& instrs, int quantumInicial, const Registers& regs)
    : pid(id), estado(PRONTO), PC(0), quantum(quantumInicial), registradores(regs.getAll()), instrucoes(instrs) {
        // std::cout << "[DEBUG] PCB criado: ID = " << pid << ", Quantum = " << quantum << ", Número de Instruções = " << instrucoes.size() << std::endl;
    }

void PCB::displayPCB() const {
    std::cout << "=============== PCB do Processo " << pid << " ===============" << std::endl << std::endl;
    std::cout << "Estado: ";
    switch (estado) {
